...
ManiacPlayer::ManiacPlayer(ConfigFile *c, int id, unsigned uniqueId, PlayerType type, std::string name, 
	...
	Tools::GetRand(14, 17, 1, &utgFullTableRange);
...
bool ManiacPlayer::flopShouldBet(){
	...
	// if pretty good hand
	if (myFlopHandSimulation.winRanged > 0.5){
		...
		if (rand == 1 && ! bHavePosition)
			return false;// will probably check raise
		...
		// if I was the last raiser preflop, I may bet with nothing
		if (lastRaiserID == myID &&  runningPlayers->size() < 4){
...
bool ManiacPlayer::flopShouldCall(){
	...
	if (myFlopHandSimulation.winRanged * 100 < getPotOdd() * 0.7)
		return false;
...
bool ManiacPlayer::flopShouldRaise(){
	...
	if (myFlopHandSimulation.winRanged * 100 < getPotOdd()){
		...
		if (getPotOdd() < 40 && runningPlayers->size() < 4){
			int rand = 0;
			Tools::GetRand(1, 3, 1, &rand);
	...
	if (myFlopHandSimulation.winRanged > 0.60){
		myRaiseAmount = pot;
		return true;
	}
