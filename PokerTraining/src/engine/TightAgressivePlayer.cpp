...
TightAgressivePlayer::TightAgressivePlayer(ConfigFile *c, int id, unsigned uniqueId, PlayerType type, std::string name, 
	...
	Tools::GetRand(8, 11, 1, &utgFullTableRange);
...
bool TightAgressivePlayer::preflopShouldCall(){
	...
	if (callingRange == -1)
		return false;// ever call : raise or fold
	...
	if (currentHand->getRunningPlayerList()->size() > 2 && nbRaises + nbCalls > 1 && nbRaises < 3 && myPosition >= LATE)
...
bool TightAgressivePlayer::flopShouldBet(){
	...
	if (myFlopHandSimulation.winRanged > 0.7){
		...
		if (lastRaiserID == myID && myFlopHandSimulation.winRanged > 0.3 && runningPlayers->size() < 4){
...
bool TightAgressivePlayer::flopShouldRaise(){
	...
	if (myFlopHandSimulation.winRanged > 0.8){
		myRaiseAmount = pot;
		return true;
	}
...
bool TightAgressivePlayer::turnShouldBet(){
	...
	if (! (myTurnState.IsNoPair || myTurnState.IsMiddlePair || myTurnState.IsBottomPair)){
...
bool TightAgressivePlayer::turnShouldRaise(){
	...
	if (! (myTurnState.IsNoPair || myTurnState.IsMiddlePair || myTurnState.IsBottomPair) && nbRaises == 1){
