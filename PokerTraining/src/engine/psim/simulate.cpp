...
namespace Simulate
{
    ...
    void GetHandState(const Hand& shand, PostFlopState* state)
    {
...

        {
			...
			if (firstRank > boardRank)
				state->UsesFirst = true;

			if (secondRank > boardRank)
                state->UsesSecond = true;
       ...
       {//Grab some info about the board itself.
        ...
        if (state->IsNoPair || state->IsOnePair) //Attempt to find out what kind of one pair or nothing.
        {
            ...
            for (uint32 i = 1; true; i <<= 1) //Start with the 1 bit set for a deuce.
                if (board & i)
                {
                    lowestBoardCard = i;
                    break;
			...
			if (state->IsFullHousePossible){ // the board is paired
				state->UsesFirst = false;
				state->UsesSecond = false;
			}
            if (h1Rank == highestBoardCard){ 
                state->IsTopPair = true;
				state->UsesFirst = true;
				state->UsesSecond = false;
			}
            if (h2Rank == highestBoardCard){
                state->IsTopPair = true;
				state->UsesFirst = false;
				state->UsesSecond = true;
			}
            if (h1Rank == lowestBoardCard){
                state->IsBottomPair = true;
				state->UsesFirst = true;
				state->UsesSecond = false;
			}
            if (h2Rank == lowestBoardCard){
                state->IsBottomPair = true;
				state->UsesFirst = false;
				state->UsesSecond = true;
			}
            ...
            if (state->IsOnePair && !state->IsTopPair && !state->IsBottomPair && !state->IsPocketPair){
                state->IsMiddlePair = true;
				// TODO dermine which card is NOT used
			}
