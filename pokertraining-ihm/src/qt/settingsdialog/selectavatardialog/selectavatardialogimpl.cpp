...
// 		myItem->setLink(myAppDataPath +"gfx/gui/table/default/genereticAvatar.png");
// 		myItem->setText("PokerTraining");
...
void selectAvatarDialogImpl::refreshAvatarView()
{
	...
	for (i=0; i<currentViewList.size(); i++) {
		...
		myItem->setLink(currentViewList.at(i));
...
QString selectAvatarDialogImpl::getAvatarLink()
{
	...
	if(groupBox->isChecked()) {
		if(myItemList.size() == 1) {
			return static_cast<MyAvatarListItem*>(listWidget->currentItem())->getLink();
		} else return QString("");
	}
